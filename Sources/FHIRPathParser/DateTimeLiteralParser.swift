...
extension DateTimeLiteralParser {
    ...
    private mutating func parseDateFormat() throws(ParseError) -> Date {
        ...
        let calendar = FHIRPathCalendar.gregorian(timeZone: FHIRPathCalendar.utc)
        guard let firstOfMonth = calendar.date(from: DateComponents(year: year, month: month)),
              calendar.range(of: .day, in: .month, for: firstOfMonth)?.contains(day) == true else {
