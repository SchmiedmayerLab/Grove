...
Defines the requirements for Grove modules, including hints and examples on structuring your Grove module, Swift Package, and repository.


The keywords "MUST", "MUST NOT", "REQUIRED", "SHALL", "SHALL NOT", "SHOULD", "SHOULD NOT", "RECOMMENDED", "MAY", and "OPTIONAL" in this document are to be interpreted as described in [RFC 2119](https://www.ietf.org/rfc/rfc2119.txt).

The following guide defines the following terminology:
- **Module**: A module is the fundamental building block of the Grove ecosystem. A module can range from standalone functionality using frameworks used in Grove, such as Swift, SwiftUI, or interoperability standards, to being deeply embedded in Grove and providing complex functionality, including user interfaces and data management. The simplest way to be integrated into Grove is the Swift ``Module`` protocol. A module MAY use the ``Module`` protocol but is not REQUIRED to do so. Some modules MAY provide functionality independent of the Grove integration mechanisms. We generally refer to Grove as a _modularized_ ecosystem.
...
- **Repository**: A repository is a collection of versioned files embedded in a version control system. The Grove context contains a single Swift Package and other configuration files, such as a continuous integration (CI) setup. Repositories are typically hosted on GitHub but can also be hosted on other platforms.

To be featured by the Grove team, a module, as well as the surrounding Swift Package and repository, MUST conform to this guide.
The Grove core team determines the definitive endorsement or feature of a Grove module.
...
Feel free to link and use the Grove community guidelines and other community standards of Grove.
Please ensure you reference and credit the Grove team if you reuse any setup.
...
A Grove module MUST support the latest major Grove version within two months after the release.
The module MUST use Grove built-in features and integrate into the ecosystem of modules listed in the [package manifest](../../../Package.swift).
It is RECOMMENDED to contribute features to existing Grove modules if they fit within the feature scope rather than creating a new module.

Different distinct functionalities under the umbrella of a Grove module SHOULD be separated out into different targets.
...
Stale projects MAY not be considered in conformance with the Grove module guidelines.


## Code Style

The Grove repository MUST have a comprehensive and detailed style guide defined by a configuration file and tool.
It is RECOMMENDED to use [swiftlint](https://github.com/realm/SwiftLint) for Swift-related code style checks.

It is RECOMMENDED using the [Grove SwiftLint configuration](../../../.swiftlint.yml).
Changes to the configuration MAY be possible and are judged by the Grove core team.
...
The Grove repository MUST incorporate an automated testing setup, including unit tests and user interface (UI) tests if applicable.
