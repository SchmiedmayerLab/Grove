...
Modules can use the constraint mechanism to enforce a set of requirements to the ``Standard`` used in the Grove-based software where the module is used.
...
The standard as well as all modules, are defined using the ``Configuration`` in the app delegate conforming to ``GroveAppDelegate``.
