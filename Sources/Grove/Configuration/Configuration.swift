...
/// Defines the ``Standard`` and ``Module``s that are used in a Grove project.
///
...
///
///
/// The following example demonstrates the usage of an `ExampleStandard` standard and reusable Grove modules, including the `HealthKit` and `QuestionnaireDataSource` modules:
/// ```swift
/// import Grove
...
public struct Configuration {
    ...
    ///   - modules: The ``Module``s used in the Grove project. You can define the ``Module``s using the ``ModuleBuilder`` result builder.
