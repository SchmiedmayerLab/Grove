...
public class _StandardPropertyWrapper<Constraint> {
    ...
    /// The injected ``Standard`` that is resolved by ``Grove/Grove``.
    public var wrappedValue: Constraint {
        guard let standard else {
            preconditionFailure(
                ...
                Only access dependencies once the module has been configured and the Grove initialization is complete.
                """
            )
        }
        return standard
...
extension _StandardPropertyWrapper: GrovePropertyWrapper {
    func inject(grove: Grove) throws(GrovePropertyError) {
        switch load(grove.standard) {
        case .success(let standard):
            self.standard = standard
        case .failure:
            let standardType = type(of: grove.standard)
            throw GrovePropertyError.unsatisfiedStandardConstraint(
