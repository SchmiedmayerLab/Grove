...
public final class _DependencyPropertyWrapper<Value> { // swiftlint:disable:this type_name
    ...
    deinit {
        guard let grove = grove else {
            return
        }
        nonIsolatedUninjectDependencies(notifying: grove)
    }
}


@available(iOS 18, macOS 15, watchOS 11, *)
extension _DependencyPropertyWrapper: GrovePropertyWrapper {
    func inject(grove: Grove) {
        self.grove = grove
        dependencies.inject(grove: grove)
    }

    func clear() {
        guard let grove else {
            preconditionFailure("\(Self.self) was clear without a Grove instance available")
        }
        uninjectDependencies(notifying: grove)
    }
}
...
extension _DependencyPropertyWrapper: DependencyDeclaration {
    ...
    func uninjectDependencies(notifying grove: Grove) {
        dependencies.uninjectDependencies(notifying: grove)
    }

    func nonIsolatedUninjectDependencies(notifying grove: Grove) {
        dependencies.nonIsolatedUninjectDependencies(notifying: grove)
    }
}
...
extension _DependencyPropertyWrapper: ModuleArrayDependency where Value == [any Module] {
    ...
    /// Grove automatically injects declared ``Dependency``s within the passed ``Dependency``s in the initializer, enabling proper nesting of ``Module``s.
    ///
    /// ```swift
