...
public final class RemoteNotificationRegistrationSupport: KnowledgeSource, Sendable {
    public typealias Anchor = GroveAnchor

    private let logger = Logger(subsystem: "org.grovealliance", category: "RemoteNotificationRegistrationSupport")
...
extension Grove {
    ...
    public var remoteNotificationRegistrationSupport: RemoteNotificationRegistrationSupport {
        ...
        if let existing = grove.storage[RemoteNotificationRegistrationSupport.self] {
            support = existing
        } else {
            support = RemoteNotificationRegistrationSupport()
            grove.storage[RemoteNotificationRegistrationSupport.self] = support
        }
        return support
    }
}
