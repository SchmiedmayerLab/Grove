...
final class ServiceModuleGroup: Sendable {
    ...
    nonisolated func run() async {
        ...
        guard run else {
            logger.warning("Grove service group is already running. Make sure to only use the `grove` view modifier once in your view hierarchy.")
            return
        }

        logger.debug("Starting the Grove Service.")

        defer {
            logger.debug("Shutting down Grove.")
