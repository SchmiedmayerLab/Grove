...
public class GetLocationTask: LocationTask {
    ...
    /// A weak reference to a `GroveLocation` component, used to interact with location services.
    private weak var component: GroveLocation?

    /// Initializes a new `GetLocationTask` with a given `GroveLocation` component.
    /// - Parameter component: The `GroveLocation` component used for accessing location services.
    init(component: GroveLocation) {
        self.component = component
    }

    /// Asynchronously requests the user's current location. This method can throw an error if the `GroveLocation` component is not found.
