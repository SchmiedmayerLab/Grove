...
/// `LocationAuthorizationTask` is a class that conforms to the `LocationTask` protocol. It is designed to manage the process of requesting and handling location authorization from the user. This class interacts with `GroveLocation` component to request and monitor changes in location authorization status.
public class LocationAuthorizationTask: LocationTask {
    ...
    /// A weak reference to the `GroveLocation` component, responsible for accessing and managing location services.
    private weak var component: GroveLocation?

    /// Initializes a new `LocationAuthorizationTask` with a given `GroveLocation` component.
    /// - Parameter component: The `GroveLocation` component used for handling location authorization.
    init(component: GroveLocation) {
        self.component = component
    }

    /// Asynchronously requests permission from the user to always access their location. This method can throw an error if the `GroveLocation` component is not found.
    /// - Returns: The current `CLAuthorizationStatus`.
    /// - Throws: `LocationError.componentNotFound` if the `GroveLocation` component is not available.
    @MainActor
    ...
    /// Asynchronously requests permission from the user to access their location when the app is in use. This method can throw an error if the `GroveLocation` component is not found.
    /// - Returns: The current `CLAuthorizationStatus`.
    /// - Throws: `LocationError.componentNotFound` if the `GroveLocation` component is not available.
    @MainActor
