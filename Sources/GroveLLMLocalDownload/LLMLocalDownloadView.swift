...
/// Provides an onboarding view for downloading locally executed Grove LLMs to the device.
/// 
/// It can be combined with the GroveViews `ManagedNavigationStack` to create an easy onboarding flow within the application.
///
/// The ``LLMLocalDownloadView/init(model:downloadDescription:action:)-(_,LocalizedStringResource,_)`` initializer accepts a download description displayed in the view, the `LLMLocalModel` representing the model to be downloaded, and an action closure to move onto the next (onboarding) step.
///
