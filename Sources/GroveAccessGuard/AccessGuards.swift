...
public final class AccessGuards: Module, EnvironmentAccessible {
    ...
    public func configure() {
        // Before anything reads a passcode: an empty read from the current service would otherwise be
        // taken as "no passcode set", and the user would be offered to set a new one.
...
extension AccessGuards {
    func register(_ config: some _AccessGuardConfig) throws {
        guard !configs.contains(where: { ModelKey($0.typeErasedId) == ModelKey(config.typeErasedId) }) else {
            throw NSError(domain: "org.grovealliance.accessGuard", code: 0)
        }
