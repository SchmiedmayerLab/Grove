...
You need to add the GroveAccessGuard Swift package to
...
You use the ``AccessGuards`` module to define your app's access guards, as part of the overall Grove configuration:
```swift
import Grove
...
GroveAccessGuard will automatically manage and persist each guard's state; for ``CodeAccessGuard``s, the passcodes are securely stored in the system keychain.
...
extension AccessGuardIdentifier where AccessGuard == CodeAccessGuard {
    static let exampleAccessGuard: Self = .passcode("org.grovealliance.exampleAccessGuard")
}
```
