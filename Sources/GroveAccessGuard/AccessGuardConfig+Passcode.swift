...
extension CodeAccessGuard {
    ...
    /// ```
    ///
    /// - Tip: In the example above, if `ConsumableCodes` were changed to conform to Grove's `Module` protocol and added to the configuration,
    ///     it'd be able to access (via the [`@Dependency`](../Grove/Grove.docc/Module/Module.md) API) other Grove modules,
    ///     thereby giving the access guard validation logic full access to the entire Grove environment
    ///     and it could use e.g. GroveStorage to keep track of the consumed/available codes.
    public init(
        _ id: AccessGuardIdentifier<Self>,
        timeout: Duration = .minutes(5),
