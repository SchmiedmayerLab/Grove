...
///
/// This Grove `Module` is used to configure the `GroveAccount` framework.
...
/// using the `@Dependency` property wrapper from other Grove `Module`s.
///
...
public final class AccountConfiguration {
    ...
    /// Configure the `GroveAccount` framework with external storage.
    ///
    /// Provide an ``AccountService`` implementation that manages all account-related operations.
    ...
    private func verify(
        ...
        preconditionFailure(
            ...
            In order to proceed you may use a Standard inside your Grove Configuration that conforms to \
            `AccountStorageConstraint` which handles storage of the above-listed account values. Otherwise, you may \
            remove the above-listed account values from your GroveAccount configuration.
            """
        )
    }
}
