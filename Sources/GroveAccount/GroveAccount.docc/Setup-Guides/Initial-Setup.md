...
A quick-start guide that shows you how to set up `GroveAccount` in your App.

## Overview

This article guides you through the mandatory steps to get `GroveAccount` up and running.

### Account Configuration

Use the ``AccountConfiguration`` `Module` to configure `GroveAccount` for you application.
...
or from other Grove `Modules` using `@Dependency`.
...
```swift
class MyAppDelegate: GroveAppDelegate {
...
> Note: You may also use the ``ConfiguredAccountKey/supports(_:file:line:)-7108n`` configuration to mark a ``AccountKey`` as
