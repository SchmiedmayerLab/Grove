...
///
/// The crucial properties of a ``LLMTool`` are the ``LLMTool/name``, serving as the identifier of the function, as well as the ``LLMTool/description``,
/// enabling the LLM to understand the purpose of the available function call.
/// The actual logic that is executed upon a function call by the LLM resides in the ``LLMTool/execute()`` function.
/// ``LLMTool/execute()`` returns a `String?`, containing all the information the Grove application wants to provide to the LLM upon a function call,
/// and is automatically injected into the LLM conversation by `GroveLLM`.
...
/// Don't use the `nil` return value as an escape hatch to return no proper value in case of an exception within the function. Either properly handle the exception directly within the ``LLMTool`` or rethrow the exception.
/// The rethrown exception will then be handled by ``GroveLLMOpenAI`` and surfaced to the user.
///
/// The ``LLMTool`` can get dependencies injected by the initializing component (e.g., a `View`) via a custom-implemented initializer.
/// The protocol puts no requirements upon the shape of the initializer, enabling developers to pass in arbitrary dependencies necessary to perform the logic of the ``LLMTool``.
///
/// # Usage
///
/// The code below demonstrates a short example of the base usage of ``LLMTool``s with ``LLMOpenAISchema``.
...
public protocol LLMTool: Sendable {
    ...
    ///            In case of an exception within the function, don't use the `nil` return value but throw a proper exception that will be surfaced to the user by ``GroveLLMOpenAI``.
    func execute() async throws -> String?
}
