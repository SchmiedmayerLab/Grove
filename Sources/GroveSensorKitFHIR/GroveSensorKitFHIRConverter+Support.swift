...
extension GroveSensorKitFHIRConverter {
    ...
    static func catalogEntry(sourceToken: String) throws -> SensorKitFHIRCatalogEntry {
        guard let entry = SensorKitFHIRCatalog.current.entries.first(where: {
            $0.sourceToken == sourceToken
        }) else {
            throw GroveSensorKitFHIRRecordError.sourceTypeNotAdmitted(sourceToken)
        }
        return entry
    }
    ...
    static func sourceTypeExtension(_ code: String) -> Extension {
        Extension(
            url: FHIRPrimitive(FHIRURI(
                stringLiteral: GroveSensorKitContract.sourceTypeExtension
            )),
    ...
    static func contextExtensions(
        _ context: GroveSensorKitFHIRConversionContext,
        converterURL: String
    ) -> [Extension] {
    ...
    static func conversionProvenance(
        ...
        Provenance(
            ...
            meta: Meta(profile: [profile(GroveSensorKitContract.conversionProvenanceProfile)]),
    ...
    static func validateECG(_ record: GroveSensorKitECGRecord) throws -> ValidatedECG {
        let frequency = try decimal(record.frequencyHertz, field: "frequency", index: nil)
        guard frequency > 0 else {
            throw GroveSensorKitFHIRRecordError.invalidSamplingFrequency(record.frequencyHertz)
        }
        let periodMilliseconds = Decimal(1_000) / frequency
        guard periodMilliseconds * frequency == 1_000 else {
            throw GroveSensorKitFHIRRecordError.samplingFrequencyNotExactlyRepresentable(
        ...
        for (batchIndex, batch) in record.batches.enumerated() {
            ...
            guard offset >= 0, !batch.millivolts.isEmpty else {
                throw GroveSensorKitFHIRRecordError.invalidECGBatch(index: batchIndex)
            }
            let expectedOffset = Decimal(sampleCount) * periodSeconds
            guard offset == expectedOffset else {
                throw GroveSensorKitFHIRRecordError.nonUniformTiming(index: sampleCount)
            }
        ...
        guard duration == lastOffset else {
            throw GroveSensorKitFHIRRecordError.inconsistentECGDuration
        }
        return ValidatedECG(
    ...
    static func exactDateTime(
        ...
        guard approximate.isFinite,
            ...
            throw GroveSensorKitFHIRRecordError.nonFiniteValue(field: "date", index: nil)
        }
    ...
    static func conceptCoding(_ code: String, _ display: String) -> Coding {
        Coding(
            ...
            system: GroveSensorKitContract.conceptCodeSystem.asFHIRURIPrimitive()
        )
    }
    ...
    static func valueConcept(_ code: String, _ display: String) -> CodeableConcept {
        CodeableConcept(coding: [Coding(
            code: code.asFHIRStringPrimitive(),
            display: display.asFHIRStringPrimitive(),
            system: GroveSensorKitContract.valueCodeSystem.asFHIRURIPrimitive()
        )])
    }
