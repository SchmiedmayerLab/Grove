...
``GroveSensorKitFHIRConverter`` accepts explicit typed records and returns a complete
``GroveSensorKitFHIRConversion``. It never queries SensorKit. Every graph uses stable
...
The structured v0.2 mappings are rotation-rate SampledData, a hybrid ECG waveform plus
...
native RecordingDocument. ``SensorKitFHIRCatalog`` is generated from the IG and records
...
``GroveSensorKitNativeRecording`` therefore requires one explicit
``GroveSensorRawPayloadAdmission``: caller-authorized opaque disclosure or verified
...
- ``GroveSensorKitFHIRConverter``
- ``GroveSensorKitFHIRConversionContext``
- ``GroveSensorKitFHIRConversion``
- ``GroveSensorKitFHIRRecord``
- ``GroveSensorKitSourceRecordID``
- ``GroveSensorKitNativeRecording``
- ``GroveSensorRawPayloadAdmission``

### Authoritative catalog

- ``SensorKitFHIRCatalog``
