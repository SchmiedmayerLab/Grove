...
/// relative URLs ``GroveSensorRecordingDocument/Payload/sidecar(path:bytes:)`` uses, so
...
public struct SensorBatchArchive: Sendable {
    ...
    public enum ArchiveError: Error, Equatable, Sendable {
        ...
        /// The path is absolute, ambiguous, or could escape the archive root.
    ...
    public mutating func addFile(path: String, data: Data) throws {
        guard GroveSensorRecordingDocument.isRelativeSidecarPath(path) else {
