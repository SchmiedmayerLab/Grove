...
/// ```
///
/// The ``LLMChatViewSchema`` may also be passed a `ChatView/ChatExportFormat` to enable the chat export functionality and define the format of the to-be-exported `GroveChat/Chat`; possible export formats are `.pdf`, `.text`, and `.json`.
...
public struct LLMChatViewSchema<Schema: LLMSchema>: View {
    ...
    /// Creates a ``LLMChatViewSchema`` with an ``LLMSchema`` that provides developers with a basic chat view to interact with a Grove LLM.
    /// 
    /// - Parameters:
