...
///
/// - Tip: The ``LLMChatView`` builds on top of the [GroveChat package](../../GroveChat/GroveChat.docc/GroveChat.md).
/// For more details, please refer to the DocC documentation of the [`ChatView`](../../GroveChat/GroveChat.docc/GroveChat.md).
///
/// - Tip: To add text-to-speech capabilities to the ``LLMChatView``, use the [GroveChat package](../../GroveChat/GroveChat.docc/GroveChat.md) and more specifically the `View/speak(_:muted:)` and `View/speechToolbarButton(enabled:muted:)` view modifiers.
/// For more details, please refer to the DocC documentation of the [`ChatView`](../../GroveChat/GroveChat.docc/GroveChat.md).
///
/// ### Usage
///
...
/// The ``LLMChatView`` may also be passed a `ChatView/ChatExportFormat` to enable the chat export functionality and define the format of the to-be-exported `GroveChat/Chat`; possible export formats are `.pdf`, `.text`, and `.json`.
///
/// In order to simplify the usage of an ``LLMSession``, GroveLLM provides the ``LLMSessionProvider`` property wrapper that conveniently instantiates an ``LLMSchema`` to an ``LLMSession``.
...
///
/// In addition, one is able to use the  text-to-speech capabilities of [GroveChat package](../../GroveChat/GroveChat.docc/GroveChat.md) via the `View/speak(_:muted:)` and `View/speechToolbarButton(enabled:muted:)` view modifiers.
///
/// ```swift
...
public struct LLMChatView<Session: LLMSession>: View {
    ...
    /// Creates a ``LLMChatView`` with a `Binding` of a ``LLMSession`` that provides developers with a basic chat view to interact with a Grove LLM.
    ///
    /// - Parameters:
