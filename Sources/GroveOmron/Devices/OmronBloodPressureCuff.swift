...
@_spi(Migration)
public import GroveDevices
public import GroveNumerics
import OSLog
...
///   in `GroveDevicesUI` related components.
...
public final class OmronBloodPressureCuff: BluetoothDevice, Identifiable, OmronHealthDevice, BatteryPoweredDevice, @unchecked Sendable {
    ...
    private let logger = Logger(subsystem: "org.grovealliance.omron", category: "BloodPressureCuffDevice")
    ...
    @GroveBluetooth private var didReceiveFirstTimeNotification = false
    ...
    private func handleCurrentTimeChange(_ time: CurrentTime) async {
        ...
        // First time notification might come before we are considered fully connected (from GroveBluetooth point of view).
