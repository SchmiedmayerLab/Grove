...
private struct PairContext {
    ...
    mutating func validateContainer(
        ...
        for definition in definitions {
            ...
            let hasExpressionEnablement = !PairRules.extensions(
                definition,
...
extension PairContext {
    ...
    private mutating func validateAnswerConstraints(
        ...
        let minimum = PairRules.firstExtensionValue(
            definition,
            url: PairRules.URL.minOccurs
        ).flatMap { PairRules.integer($0.value) }
        let maximum = PairRules.firstExtensionValue(
            definition,
    ...
    private mutating func validateUnit(
        ...
        let options = PairRules.extensions(
            definition,
    ...
    private mutating func validateAttachment(
        ...
        let allowedTypes = Set(PairRules.extensions(
            definition,
            url: PairRules.URL.mimeType
        ).compactMap(PairRules.extensionValue).compactMap { $0.value as? String })
        ...
        guard let maximum = PairRules.firstExtensionValue(
            definition,
            url: PairRules.URL.maxSize
        ).flatMap({ PairRules.decimal($0.value) }) else {
            return
        }
...
private struct ValueSetResolver {
