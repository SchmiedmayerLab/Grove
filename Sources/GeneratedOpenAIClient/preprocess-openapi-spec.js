...
/**
 * Drop `Response` fields that the API sends as `null` from its `required` list.
 *
 * The spec marks `error`, `incomplete_details` and `instructions` required without marking them nullable, so the
 * generator emits non-optional properties and every successful non-streamed response fails to decode.
 */
function fixNullableRequiredResponseFields(obj) {
  const nullableFields = ["error", "incomplete_details", "instructions"];
  const variants = obj?.components?.schemas?.Response?.allOf ?? [];
  variants.forEach((variant) => {
    if (Array.isArray(variant?.required)) {
      variant.required = variant.required.filter((field) => !nullableFields.includes(field));
    }
  });
  return obj;
}
 ...
 */
function addOneOfDiscriminatorMappings(rootDoc, schemaNames) {
  ...
  function walk(node, name) {
    ...
    if (wanted.has(name) && Array.isArray(node.oneOf) && node.discriminator?.propertyName && !node.discriminator.mapping) {
...
openapiDoc = fixNullableRequiredResponseFields(openapiDoc);
...
// Only for the unions we have deliberately mapped and updated the call sites for. Running this over the whole
// spec would rename every generated case (`.FunctionTool` becomes `.function`), which is a breaking refactor —
// but leaving these four unmapped is worse: a union whose discriminator has no mapping matches on schema name
// rather than on the `type` the wire sends, so decoding fails on data the API really returns. Keeping the list
// here means regenerating the client cannot quietly drop the fix.
openapiDoc = addOneOfDiscriminatorMappings(openapiDoc, [
  "Annotation",
  // Inline union under `WebSearchToolCall.action`, matched by its property name rather than a schema name.
  "action",
  "OutputItem",
