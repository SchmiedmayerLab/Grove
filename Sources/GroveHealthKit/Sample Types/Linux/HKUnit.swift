...
public final class _HKUnit: NSObject, Sendable {
    private static let nullUnit = _HKUnit(
        // NOTE: HealthKit represented the null unit as having a factorization of `null: 1`, which made
        // it compatible with any other unitless dimension. We diverged, giving it an empty
        // factorization. Apple changed to an empty factorization in OS 27, so the two now agree —
        // keep this as it is.
        factorization: .init([:]),
        dimension: .null,
        scaleOffset: 0,
