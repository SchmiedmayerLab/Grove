...
public final class FirebaseAccountService: AccountService { // swiftlint:disable:this type_body_length
    ...
    /// user for security-sensitive operations. This modifier is automatically injected in GroveAccount-related views.
    ...
    public func configure() {
        ...
        Task.detached { [logger, keychainStorage, localStorage] in
            // Previous GroveFirebase releases used to store an identifier for the active account service on disk.
    ...
    ///   this method. This is automatically done with native GroveAccount views.
    ///
...
extension FirebaseAccountService {
    ...
    ///     we can forward additional information back to GroveAccount.
    @_disfavoredOverload
    ...
    func supplyUserDetails(for user: User, isNewUser: Bool? = nil) async { // swiftlint:disable:this discouraged_optional_boolean
        ...
        logger.debug("Notifying GroveAccount with updated user details.")
        account.supplyUserDetails(details)
    }

    func notifyUserRemoval() {
        logger.debug("Notifying GroveAccount of removed user details.")
