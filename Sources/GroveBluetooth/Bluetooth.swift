...
///
/// Once you have the `Bluetooth` module configured within your Grove app, you can access the module within your
...
/// ```
///
/// ### Integration with Grove Modules
///
/// A Grove [`Module`](../Grove/Grove.docc/Module/Module.md) is a great way of structuring your application into
...
/// configured within the Grove application.
///
/// ```swift
...
///
///     // declare dependency to a configured Grove Module
...
public final class Bluetooth: Module, EnvironmentAccessible, @unchecked Sendable {
    ...
    nonisolated static let logger = Logger(subsystem: "org.grovealliance.bluetooth", category: "Bluetooth")
    ...
    @GroveBluetooth private var _lazy_discoveryConfiguration: Set<DiscoveryDescription>?
    // swiftlint:disable:previous discouraged_optional_collection identifier_name
    @GroveBluetooth private var discoveryConfiguration: Set<DiscoveryDescription> {
    ...
    @GroveBluetooth private var initializedDevices: OrderedDictionary<UUID, any AnyWeakDeviceReference> = [:]
    ...
    private func handleUpdatedNearbyDevicesChange(_ discoveredDevices: OrderedDictionary<UUID, BluetoothPeripheral>) {
        var newlyPreparedDevices: Set<UUID> = [] // track for which device instances we need to call Grove/loadModule(...)
        ...
        Task { @MainActor [newlyPreparedDevices] in
            ...
            for (uuid, device) in discoveredDeviceInstances where nearbyDevices[uuid] == nil {
                ...
                if newlyPreparedDevices.contains(uuid) {
                    // We load the module with external ownership. Meaning, Grove won't keep any strong references to the Module and deallocation of
                    // the module is possible, freeing all Grove related resources.
                    grove.loadModule(device, ownership: .external)
                }
            }
    ...
    public func retrieveDevice<Device: BluetoothDevice>(
        ...
        // We load the module with external ownership. Meaning, Grove won't keep any strong references to the Module and deallocation of
        // the module is possible, freeing all Grove related resources.
        await grove.loadModule(device, ownership: .external)
