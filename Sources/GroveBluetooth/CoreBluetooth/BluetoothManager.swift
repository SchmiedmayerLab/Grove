...
public class BluetoothManager: Observable, Sendable, Identifiable { // swiftlint:disable:this type_body_length
    private let logger = Logger(subsystem: "org.grovealliance.bluetooth", category: "BluetoothManager")
    ...
    func supplyCBCentral() -> CBCentralManager {
        ...
        let central = CBCentralManager(
            ...
            queue: GroveBluetooth.shared.dispatchQueue,
    ...
    deinit {
        ...
        Task { @GroveBluetooth [storage, _centralManager, isScanning, logger] in
            if isScanning {
...
extension BluetoothManager {
    private class Delegate: NSObject, CBCentralManagerDelegate {
        private let logger = Logger(subsystem: "org.grovealliance.bluetooth", category: "BluetoothManagerDelegate")
        ...
        func centralManagerDidUpdateState(_ central: CBCentralManager) {
            ...
            // So whats the solution? We schedule onto a background SerialExecutor (@GroveBluetooth) so we maintain execution
            ...
            GroveBluetooth.assumeIsolated { [logger] in
        ...
        func centralManager(
            ...
            GroveBluetooth.assumeIsolated { [logger, data] in
