...
public class BluetoothPeripheral { // swiftlint:disable:this type_body_length
    private let logger = Logger(subsystem: "org.grovealliance.bluetooth", category: "BluetoothDevice")
        ...
        try await withThrowingDiscardingTaskGroup { group in
            for description in descriptions {
                ...
                if description.discoverDescriptors {
                    ...
                    // Currently descriptor interactions aren't really supported by GroveBluetooth. However, we support the initial
    ...
    deinit {
        ...
        Task.detached { @Sendable @GroveBluetooth [storage, nearby] in
...
extension BluetoothPeripheral {
    private class Delegate: NSObject, CBPeripheralDelegate {
        private let logger = Logger(subsystem: "org.grovealliance.bluetooth", category: "BluetoothDeviceDelegate")
