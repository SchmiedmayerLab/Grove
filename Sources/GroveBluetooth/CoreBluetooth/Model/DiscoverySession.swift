...
class DiscoverySession: Sendable {
    private let logger = Logger(subsystem: "org.grovealliance.bluetooth", category: "DiscoverySession")
...
extension DiscoverySession {
    ...
    func kickOffAutoConnect() {
        ...
        let item = BluetoothWorkItem { [weak self] in
            ...
            connectionAttempt = Task { @GroveBluetooth [weak self] in
                defer {
...
extension DiscoverySession {
    ...
    func scheduleStaleTask(for device: BluetoothPeripheral, withTimeout timeout: TimeInterval) {
        ...
        timer.schedule(for: timeout, in: GroveBluetooth.shared.dispatchQueue)
    }
