...
public enum BluetoothError: Error, CustomStringConvertible, LocalizedError {
    ...
    ///
    /// This error is used as a fallback only. If the device disconnected with an error, GroveBluetooth will forward the error and only use this error if no other error or reason is available.
