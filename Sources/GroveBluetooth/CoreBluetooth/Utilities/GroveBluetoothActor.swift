...
/// on the GroveBluetooth actor's dispatch queue and therefore are never sent over actor boundaries.
...
struct CBInstance<Value>: Sendable {
    ...
    @GroveBluetooth var cbObject: Value {
        object
    }
    ...
    @GroveBluetooth subscript<T>(dynamicMember keyPath: KeyPath<Value, T>) -> T {
...
///
/// GroveBluetooth synchronizes all its state to the `GroveBluetooth` global actor.
/// The `GroveBluetooth` global actor is used to schedule all Bluetooth related tasks and synchronize all Bluetooth related state.
...
/// ### Data Safety
///
/// Some read-only state of `GroveBluetooth` is deliberately made non-isolated and can be accessed from any thread (e.g., ``Bluetooth/state`` or ``Bluetooth/isScanning`` properties).
...
///
/// If you need a consistent view of your Bluetooth peripheral's state, especially if you access multiple properties at the same time, consider isolating to the `@GroveBluetooth` global actor.
///
/// All accessor bindings of GroveBluetooth property wrappers (a call like `deviceInformation.$manufacturerName` using ``Characteristic/projectedValue``) capture the current
...
@available(iOS 18, macOS 15, watchOS 11, *)
@globalActor
public actor GroveBluetooth {
    /// The shared actor instance.
    public static let shared = GroveBluetooth()
    ...
    private init() {
        let dispatchQueue = DispatchQueue(label: "org.grovealliance.bluetooth", qos: .userInitiated)
...
extension GroveBluetooth {
    ...
    public static func assumeIsolated<T: Sendable>(
        _ operation: @GroveBluetooth () throws -> T,
        ...
        typealias YesActor = @GroveBluetooth () throws -> T
