...
struct TaskRun: Identifiable {
    ...
    func groupHeadings(otherThan pageTitle: String, in language: String?) -> [String] {
        openedGroups.compactMap { group in
            let title = group.title.resolved(in: language)
            let heading = title.isEmpty ? (group.shortTitle?.resolved(in: language) ?? "") : title
