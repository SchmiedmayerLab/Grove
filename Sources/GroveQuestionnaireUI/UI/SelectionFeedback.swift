...
enum SelectionFeedback {
    ...
    static let deselection: Animation = .easeOut(duration: 0.08)
    ...
    /// instead of queueing.
    static let advanceDelay: Duration = .milliseconds(80)
    ...
    static func record(
        ...
        // Moving on is timed against the confirmation rather than driven by its completion handler:
        // where nothing animates — a UI test host among them — that handler never runs, and the
        // participant is left on a page that should have moved.
        withAnimation(confirmation, apply)
        Task { @MainActor in
