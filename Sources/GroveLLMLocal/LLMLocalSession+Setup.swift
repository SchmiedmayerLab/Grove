...
extension LLMLocalSession {
    ...
    internal func _setup(continuation: AsyncThrowingStream<String, any Error>.Continuation?) async -> Bool {
        ...
        guard verifyModelDownload() else {
            ...
            Self.logger.error("GroveLLMLocal: Local LLM file could not be opened, indicating that the model file doesn't exist")
            return false
        }
        ...
        do {
            ...
            Self.logger.error("GroveLLMLocal: Failed to load local `modelContainer`")
            return false
        }
        return true
#endif
