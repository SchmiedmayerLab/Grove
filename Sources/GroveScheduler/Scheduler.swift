...
///
/// Below is a example on how to create your own [`Module`](../Grove/Grove.docc/Module/Module.md)
...
public final class Scheduler: Module, EnvironmentAccessible, DefaultInitializable, Sendable { // swiftlint:disable:this type_body_length
    ...
    public enum PersistenceConfiguration: Sendable {
        ...
        /// - parameter directory: the directory where GroveScheduler should store its data.
        ...
        /// The ``Scheduler`` will use an on-disk database for persistence, in the app's Application Support container.
        ///
        /// Deliberately not `@inlinable`: an inlinable body is emitted into the consuming app's binary,
        /// which would freeze the path derivation at their compile time.
        public static var onDisk: Self {
            do {
                return .onDisk(directory: try StorageNamespace.app.directory(.scheduler))
            } catch {
                preconditionFailure(
                    """
                    Unable to determine where the \(Scheduler.self) should store its data: \(error)
                    Pass an explicit persistence configuration, e.g. `.onDisk(directory:)`.
                    """
                )
            }
        }
    ...
    nonisolated public init(persistence: PersistenceConfiguration) {
        ...
        case .onDisk(let directory):
            ...
            // Relocation must precede directory creation: a store is committed by renaming a staging
            // directory onto the destination, which cannot happen if the destination already exists.
            guard let persistentStorageUrl = Self.prepareStorage(in: directory) else {
                // The store could not be brought forward safely. Refusing to open anything is the only
                // option that leaves the user's data recoverable on a later launch.
                _container = .failure(DataError.invalidContainer(SchedulerStorageUnavailableError()))
                return
            }
    ...
    public func configure() {
        ...
        // It also makes it easier to understand the SwiftData-related infrastructure around Grove Scheduler.
...
@available(iOS 18, macOS 15, watchOS 11, *)
extension Scheduler {
    /// Marks that the iOS 26 localization migration has run. Travels with the store; see ``prepareStorage(in:)``.
