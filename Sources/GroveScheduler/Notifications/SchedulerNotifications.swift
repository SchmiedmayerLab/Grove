...
///     [`GroveAppDelegate`](../../Grove/Grove.docc/Grove.md).
///
...
/// Apple limits the total pending notification requests to `64` per application. GroveScheduler, by default, doesn't schedule more than `30` local notifications at a time for events
...
///     capability and enable the **Background fetch** option. GroveScheduler automatically schedules background tasks to update the scheduled notifications.
...
///
/// By default, GroveScheduler will try to request [provisional notification authorization](https://developer.apple.com/documentation/usernotifications/asking-permission-to-use-notifications#Use-provisional-authorization-to-send-trial-notifications).
...
///     you requested notification authorization from the user. Otherwise, GroveScheduler won't schedule notifications properly.
///
/// ### Notifications for Completed Events
///
/// When notifications are enabled for a task, the Grove Scheduler module sends alerts based on the task’s schedule.
...
///
/// You can define a function to preemptively complete a set of upcoming events, preventing notifications from being triggered by the Grove Scheduler or other application logic.
...
public final class SchedulerNotifications: Module, DefaultInitializable, EnvironmentAccessible, Sendable {
    ...
    ///
    /// This options limits the maximum amount of local notifications request that GroveScheduler schedules.
    ///
    /// - Note: Default is `30`.
    ...
    func registerProcessingTask(using scheduler: Scheduler) {
        if Self.backgroundFetchEnabled {
            ...
            backgroundTaskRegistered = BGTaskScheduler.shared.register(
                forTaskWithIdentifier: PermittedBackgroundTaskIdentifier.resolved.rawValue,
                using: .main
...
extension SchedulerNotifications {
    ...
    private func scheduleNotifications(for scheduler: borrowing Scheduler) async throws {
        // 1: remove all pending notification requests scheduled by GroveScheduler
        await notifications.removePendingNotificationRequests { request in
            request.isGroveSchedulerRequest
        }
...
extension SchedulerNotifications: NotificationHandler {
    public func receiveIncomingNotification(_ notification: UNNotification) async -> UNNotificationPresentationOptions? {
        guard notification.request.isGroveSchedulerRequest else {
...
extension SchedulerNotifications {
    ...
    private func scheduleNotificationsRefresh(nextThreshold: Date = .nextWeek) {
        ...
        if backgroundTaskRegistered {
            ...
            #else
            let request = BGAppRefreshTaskRequest(identifier: PermittedBackgroundTaskIdentifier.resolved.rawValue)
            ...
            do {
                ...
                if case .unavailable = error.code {
                    logger.warning(
                            """
                            Failed to schedule notifications processing task for GroveScheduler: \
...
#endif
                logger.error("Failed to schedule notifications processing task for GroveScheduler: \(error.code)")
            } catch {
                logger.error("Failed to schedule notifications processing task for GroveScheduler: \(error)")
            }
            #endif
        } else {
...
extension LocalStorageKeys {
    fileprivate static let legacyTasks = LocalStorageKey<[LegacyTaskModel]>(
        // From the vault: scheduler 1.0 wrote here, and a rename would orphan those notifications.
...
@available(iOS 18, macOS 15, watchOS 11, *)
extension SchedulerNotifications {
    /// Cancel scheduled and delivered notifications of the legacy GroveScheduler 1.0
...
extension UNNotificationRequest {
    var isGroveSchedulerRequest: Bool {
        SchedulerNotifications.isSchedulerNotification(identifier)
    }
}
