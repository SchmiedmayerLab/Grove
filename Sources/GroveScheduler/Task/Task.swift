...
///
/// - Note: GroveScheduler can automatically schedule notifications for your events. Refer to the documentation of the ``SchedulerNotifications`` module for more information.
///
/// ### Versioning
...
extension Task {
    ...
    /// Unsafely updates the task's ``id``.
    ///
    /// - Important: Only call this from a store migration, and rewrite every version of a chain in the
    ///     same transaction — `id` is unique in combination with `nextVersion`.
    @_spi(APISupport)
    public func unsafelyUpdateId(to newId: String) {
        self.id = newId
    }
}
