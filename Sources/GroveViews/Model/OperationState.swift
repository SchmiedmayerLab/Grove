...
/// Provides a meta model for the current state of a specific action or task conducted within the Grove ecosystem.
///
...
/// To effectively manage the UI's state in the Grove framework, the ``OperationState`` can be represented as a ``ViewState``.
/// This bridging mechanism allows Grove to monitor and respond to changes in the view's state, for example via the ``SwiftUICore/View/viewStateAlert(state:)-(Binding<ViewState>)`` view modifier.
///
...
/// > is required (e.g., no use of the ``SwiftUICore/View/viewStateAlert(state:)-(Binding<ViewState>)`` view modifier), one is able to omit the separately defined ``ViewState``
