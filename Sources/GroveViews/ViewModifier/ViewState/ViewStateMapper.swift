...
extension View {
    ...
    /// ```
    ///
    /// - Note: The ``OperationState`` documentation contains a complete example using the ``SwiftUICore/View/map(state:to:)`` view modifier.
    ///
    /// > Tip:
    ...
    ///
    /// - Parameters:
    ///    - operationState: The source ``OperationState`` that should be mapped to the GroveViews ``ViewState``.
