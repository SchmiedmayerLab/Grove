...
public enum LLMRealtimeTurnDetectionSettings: Encodable, Sendable {
    ...
    public struct ServerVAD: Encodable, Sendable {
        ...
        /// Amount of audio to include before the VAD detected speech. Defaults to 300ms.
        let prefixPadding: Duration
        /// Duration of silence to detect speech stop. Defaults to 500ms.
        ///
        ...
        /// Allow new speech to interrupt and stop the model’s current response (requires `createResponse` to be `true`)
        let interruptResponse: Bool
        
        /// Creates a new `ServerVAD` configuration.
        ///
        /// - Parameters:
        ///   - threshold: Activation threshold for VAD (0.0 to 1.0). A higher threshold will require louder audio to activate the model, and thus might perform better in noisy environments.
        ///   - prefixPadding: Amount of audio to include before the VAD detected speech.
        ///   - silenceDuration: Duration of silence to detect speech stop. With shorter values the model will respond more quickly, but may jump in on short pauses from the user.
        ///   - createResponse: Whether or not to automatically generate a response when a VAD stop event occurs.
        ///   - interruptResponse: Allow new speech to interrupt and stop the model’s current response (requires `createResponse` to be `true`).
        public init(
    ...
    public struct SemanticVAD: Encodable, Sendable {
        ...
        /// Creates a new `SemanticVAD` configuration.
        ///
        /// - Parameters:
        ///   - eagerness: Controls how eager the model is to interrupt the user.
        ///   - createResponse: Whether or not to automatically generate a response when a VAD stop event occurs.
        ///   - interruptResponse: Allow new speech to interrupt and stop the model’s current response (requires `createResponse` to be `true`).
    ...
    public static let `default`: LLMRealtimeTurnDetectionSettings = .semantic(.init())
    ...
    public func encode(to encoder: any Encoder) throws {
        ...
        case .server(let config):
            ...
            try container.encode(config.prefixPadding.milliseconds, forKey: .prefixPaddingMs)
            try container.encode(config.silenceDuration.milliseconds, forKey: .silenceDurationMs)
