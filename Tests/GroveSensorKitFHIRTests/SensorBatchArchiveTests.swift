...
struct SensorBatchArchiveTests {
    ...
    @Test
    func unsafeArchivePathsFailClosed() {
        var archive = SensorBatchArchive()
        #expect(throws: SensorBatchArchive.ArchiveError.invalidPath("../outside.json")) {
            try archive.addFile(path: "../outside.json", data: Data("{}".utf8))
        }
        #expect(throws: SensorBatchArchive.ArchiveError.invalidPath("payloads//ambiguous.json")) {
            try archive.addFile(path: "payloads//ambiguous.json", data: Data("{}".utf8))
        }
    }
}
