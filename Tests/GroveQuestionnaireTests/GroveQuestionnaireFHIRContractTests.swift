...
struct GroveQuestionnaireFHIRContractTests {
    ...
    func builderProducesTheExactVersionedPair() throws {
        ...
        #expect(pair.questionnaire.meta?.profile == [GroveFHIRProfile.groveQuestionnaire])
        #expect(pair.response.meta?.profile == [GroveFHIRProfile.groveQuestionnaireResponse])
        ...
        #expect(GroveQuestionnaireFHIRPairValidator().issues(
    ...
    func repositoryIDsAreOnlyWrittenWhenExplicitlyAssigned() throws {
        let questionnaireID = try GroveFHIRRepositoryID("questionnaire-42")
        let responseID = try GroveFHIRRepositoryID("response-42")
        let pair = try GroveQuestionnaireFHIRBuilder().pair(
            from: responses(),
    ...
    func pairValidationRejectsCanonicalDrift() throws {
        let valid = try GroveQuestionnaireFHIRBuilder().pair(
            from: responses(),
            authored: questionnaireResponseTestAuthoredAt
        )
        ...
        let issues = GroveQuestionnaireFHIRPairValidator().issues(
            questionnaire: valid.questionnaire,
            response: drifted
        )
        #expect(issues.contains { $0.code == .questionnaireCanonical })
        #expect(throws: GroveQuestionnaireFHIRContractError.self) {
            try GroveQuestionnaireFHIRPair(
                questionnaire: valid.questionnaire,
                response: drifted
            )
        }
