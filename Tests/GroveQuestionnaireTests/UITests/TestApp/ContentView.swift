...
struct ContentView: View {
    struct WrappedQuestionnaire: Hashable, Identifiable {
        ...
        let grove: GroveQuestionnaire.Questionnaire
        ...
        init(r4 questionnaire: ModelsR4.Questionnaire) throws {
            ...
            self.grove = try .init(questionnaire)
        }
        
        init(grove questionnaire: GroveQuestionnaire.Questionnaire) {
            self.r4 = nil
            self.grove = questionnaire
        }
    }
    ...
    @State private var activeQuestionnaireNewImpl: GroveQuestionnaire.Questionnaire?
    ...
    private func loadedQuestionnaireSection(for loadedQuestionnaire: WrappedQuestionnaire) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            Text(loadedQuestionnaire.grove.metadata.title)
                .font(.headline)
            if let url = loadedQuestionnaire.grove.metadata.url?.absoluteString {
                Text(url)
                    .font(.footnote)
                    .foregroundStyle(.secondary)
            }
        ...
        .padding(.top, 4)
        Button {
            activeQuestionnaireNewImpl = loadedQuestionnaire.grove
        } label: {
            Label("Start Questionnaire (Grove Impl)", systemImage: "play.circle")
        }
        Button {
...
extension ContentView {
    private struct PredefinedQuestionnairePicker: View {
        ...
        var body: some View {
            NavigationStack {
                Form {
                    ...
                    Section("Research") {
                        menuButton(title: "PHQ-9 (Native)", questionnaire: GroveQuestionnaire.Questionnaire.phq9)
                        menuButton(title: "PHQ-9 (FHIR)", questionnaire: ModelsR4.Questionnaire.phq9)
                        menuButton(title: "GAD-7 (Native)", questionnaire: GroveQuestionnaire.Questionnaire.gad7)
        ...
        private func menuButton(title: String, questionnaire: GroveQuestionnaire.Questionnaire) -> some View {
            Button(title) {
                dismiss()
                onSelection(.init(grove: questionnaire))
            }
        }
