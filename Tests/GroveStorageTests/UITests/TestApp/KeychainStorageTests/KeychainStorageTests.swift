...
import Foundation
import Grove
@_spi(Internal) import GroveKeychainStorage
import XCTestApp


/// The app's own keychain access group, probed rather than hard-coded.
///
/// The prefix is the Apple Developer Team ID, which differs per developer, so baking one in means the
/// suite only passes for whoever set it. The entitlement is already `$(AppIdentifierPrefix)$(PRODUCT_BUNDLE_IDENTIFIER)`;
/// this reads back what the system actually assigned.
let defaultKeychainAccessGroup: String = {
    let probe: [String: Any] = [
        kSecClass as String: kSecClassGenericPassword,
        kSecAttrService as String: "org.grovealliance.keychainAccessGroupProbe",
    ...
    guard SecItemCopyMatching(query as CFDictionary, &result) == errSecSuccess,
        ...
        // Falling back to the literal keeps the failure legible rather than crashing the whole app.
        return "<unknown>.\(Bundle.main.bundleIdentifier ?? "app")"
    }
    return group
}()
...
final class KeychainStorageTests: TestAppTestCase { // swiftlint:disable:this type_body_length
    ...
    func testDeleteCredentials() throws {
        ...
        let serverCredentials1 = Credentials(username: "@Schmiedmayer", password: "GroveInventor")
        ...
        let serverCredentials2 = Credentials(username: "Stanford Grove", password: "Paul")
        try keychainStorage.store(serverCredentials2, for: .genericPassword(forService: "groveLogin"))
        try XCTAssertCredentialsMainPropertiesEqual(
            serverCredentials2,
            try XCTUnwrap(try keychainStorage.retrieveCredentials(withUsername: "Stanford Grove", for: .genericPassword(forService: "groveLogin")))
        )
        ...
        try XCTAssertEqual(
            try XCTUnwrap(keychainStorage.retrieveAllCredentials(withUsername: nil, for: .genericPassword(forService: "groveLogin"))).count,
            0
        )
    ...
    func testGenericCredentials() throws { // swiftlint:disable:this function_body_length
        ...
        let groveLoginTagNoSync = CredentialsTag.genericPassword(forService: "groveLogin", storage: .keychain)
        let groveLoginTagYesSync = CredentialsTag.genericPassword(forService: "groveLogin", storage: .keychainSynchronizable)
        let apodiniCredentialsTag = CredentialsTag.genericPassword(forService: "apodini")
        
        var serverCredentials = Credentials(username: "@PSchmiedmayer", password: "GroveInventor")
        try keychainStorage.store(serverCredentials, for: groveLoginTagNoSync)
        try XCTAssertFalse(
            try XCTUnwrap(try keychainStorage.retrieveCredentials(withUsername: "@PSchmiedmayer", for: groveLoginTagNoSync)).synchronizable
        )
        try keychainStorage.store(.init(username: "lukas", password: "psstthisissecret"), for: apodiniCredentialsTag)
        do {
            let credentials = try XCTUnwrap(try keychainStorage.retrieveCredentials(withUsername: "@PSchmiedmayer", for: groveLoginTagNoSync))
            try XCTAssertNotNil(credentials.asGenericCredentials)
            try XCTAssertNil(credentials.asInternetCredentials)
            try XCTAssertEqual(try XCTUnwrap(credentials.asGenericCredentials).kind, .genericPassword(service: "groveLogin"))
        }
        try keychainStorage.store(serverCredentials, for: groveLoginTagYesSync)
        try XCTAssertTrue(
            try XCTUnwrap(try keychainStorage.retrieveCredentials(withUsername: "@PSchmiedmayer", for: groveLoginTagYesSync)).synchronizable
        )
        try keychainStorage.store(serverCredentials, for: groveLoginTagYesSync) // Overwrite existing credentials
        try XCTAssertTrue(
            try XCTUnwrap(try keychainStorage.retrieveCredentials(withUsername: "@PSchmiedmayer", for: groveLoginTagYesSync)).synchronizable
        )
        ...
        try XCTAssertCredentialsMainPropertiesEqual(
            serverCredentials,
            try XCTUnwrap(try keychainStorage.retrieveCredentials(withUsername: "@PSchmiedmayer", for: groveLoginTagNoSync))
        )
        try XCTAssertCredentialsMainPropertiesEqual(
            serverCredentials,
            try XCTUnwrap(try keychainStorage.retrieveCredentials(withUsername: "@PSchmiedmayer", for: groveLoginTagYesSync))
        )
        try XCTAssertEqual(
            try XCTUnwrap(try keychainStorage.retrieveCredentials(withUsername: "@PSchmiedmayer", for: groveLoginTagNoSync)),
            try XCTUnwrap(try keychainStorage.retrieveCredentials(withUsername: "@PSchmiedmayer", for: groveLoginTagNoSync))
        ...
        do {
            ...
            credentialsSet.insert(try XCTUnwrap(try keychainStorage.retrieveCredentials(withUsername: "@PSchmiedmayer", for: groveLoginTagNoSync)))
            try XCTAssertEqual(credentialsSet.count, 1)
            credentialsSet.insert(try XCTUnwrap(try keychainStorage.retrieveCredentials(withUsername: "@PSchmiedmayer", for: groveLoginTagNoSync)))
            try XCTAssertEqual(credentialsSet.count, 1)
            
            credentialsSet.insert(try XCTUnwrap(try keychainStorage.retrieveCredentials(withUsername: "@PSchmiedmayer", for: groveLoginTagYesSync)))
            try XCTAssertEqual(credentialsSet.count, 2)
            credentialsSet.insert(try XCTUnwrap(try keychainStorage.retrieveCredentials(withUsername: "@PSchmiedmayer", for: groveLoginTagYesSync)))
            try XCTAssertEqual(credentialsSet.count, 2)
        }
        
        serverCredentials = Credentials(username: "@Grove", password: "Paul")
        try keychainStorage.updateCredentials(withUsername: "@PSchmiedmayer", for: groveLoginTagYesSync, with: serverCredentials)
        
        let retrievedUpdatedCredentials = try XCTUnwrap(keychainStorage.retrieveCredentials(withUsername: "@Grove", for: groveLoginTagYesSync))
        ...
        try XCTAssertEqual(try keychainStorage.retrieveAllGenericCredentials(forService: "groveLogin").count, 2)
        try XCTAssertEqual(try keychainStorage.retrieveAllGenericCredentials(forService: "apodini").count, 1)
        
        try keychainStorage.deleteCredentials(withUsername: "@PSchmiedmayer", for: groveLoginTagNoSync)
        try keychainStorage.deleteCredentials(withUsername: "@Grove", for: groveLoginTagYesSync)
        try XCTAssertNil(try keychainStorage.retrieveCredentials(withUsername: "@Grove", for: groveLoginTagYesSync))
    ...
    func testInternetCredentials() throws {
        ...
        try XCTAssertEqual(retrievedCredentials.accessGroup, defaultKeychainAccessGroup)
        ...
        let retrievedUpdatedCredentials = try XCTUnwrap(keychainStorage.retrieveCredentials(withUsername: "@Grove", for: twitterCredentialsKey))
        try XCTAssertCredentialsMainPropertiesEqual(serverCredentials, retrievedUpdatedCredentials)
        
        try keychainStorage.deleteCredentials(withUsername: "@Grove", for: twitterCredentialsKey)
        try XCTAssertNil(try keychainStorage.retrieveCredentials(withUsername: "@Grove", for: twitterCredentialsKey))
    }
    ...
    func testMultipleInternetCredentials() throws {
        ...
        let serverCredentials1 = Credentials(username: "Paul Schmiedmayer", password: "GroveInventor")
        ...
        try XCTAssertNotEqual(retrievedCredentials[0], Credentials(username: "Paul Schmiedmayer", password: "GroveInventor"))
        ...
        try keychainStorage.deleteCredentials(withUsername: "Stanford Grove", for: linkedInCredentialsKey)
    ...
    func testMultipleCredentials() throws {
        ...
        try keychainStorage.deleteCredentials(withUsername: "Stanford Grove", for: stanfordCredentialsTag)
    ...
    func testKeys0() throws {
        ...
        try XCTAssertEqual(key.accessGroup, defaultKeychainAccessGroup)
    ...
    func testKeys() throws {
        ...
        if SecureEnclave.isAvailable {
            ...
            let plainText = Data("Grove & Paul Schmiedmayer".utf8)
    ...
    func testKeys2() throws {
        var storageOptionsToTest: [KeychainItemStorageOption] = [
            ...
            .keychain(requireUserPresence: false, accessGroup: defaultKeychainAccessGroup),
            .keychain(requireUserPresence: true, accessGroup: nil),
            .keychain(requireUserPresence: true, accessGroup: defaultKeychainAccessGroup),
            .keychainSynchronizable(accessGroup: nil),
            .keychainSynchronizable(accessGroup: defaultKeychainAccessGroup)
        ]
