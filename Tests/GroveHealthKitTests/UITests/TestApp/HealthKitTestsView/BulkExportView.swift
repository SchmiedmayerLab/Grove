...
extension NSPredicate {
    nonisolated static var isGroveTestingSample: NSPredicate {
        HKQuery.predicateForObjects(
            withMetadataKey: HKSampleMetadataKeyIsGroveTestingData,
...
private struct AddHistoricalSamplesSection: View {
    ...
    var body: some View {
        ...
        AsyncButton("Delete Grove-created Historical Testing Data", role: .destructive, state: $viewState) {
            func imp<Sample>(_ sampleType: some AnySampleType<Sample>) async throws {
                let sampleType = SampleType(sampleType)
                let samples = try await healthKit.query(sampleType, timeRange: .ever, predicate: .isGroveTestingSample)
        ...
        @Sendable nonisolated func imp(
            ...
            for day in days {
                let sample = HKQuantitySample(
                    ...
                    metadata: [
                        HKSampleMetadataKeyIsGroveTestingData: "YES"
                    ]
                )
