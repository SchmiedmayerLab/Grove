...
struct FHIRPathEvaluatorTests {
    ...
    /// A response with one coded answer carrying the named scoring extensions.
    private static func codingContext(extensions: [(name: String, weight: Int)]) throws -> FHIRPathEvaluationContext {
        let markers = extensions
            .map { #"{ "url": "http://hl7.org/fhir/StructureDefinition/\#($0.name)", "valueDecimal": \#($0.weight) }"# }
            .joined(separator: ", ")
        let json = """
        {
        ...
        return FHIRPathEvaluationContext(focus: [.object(node)], constants: ["resource": [.object(node)]], clock: clock)
    }

    private func evaluate(_ expression: String) throws -> [FHIRPathValue] {
        try evaluate(expression, context: .init(clock: Self.clock))
    }
    ...
    func dateArithmetic() throws {
        ...
        let fixed = try #require(calendar.date(from: DateComponents(year: 2026, month: 8, day: 14, hour: 23)))
        let context = FHIRPathEvaluationContext(clock: FHIRPathClock(instant: fixed, timeZone: timeZone))
        #expect(try evaluate("today()", context: context) == [.date(DateComponents(year: 2026, month: 8, day: 14))])
        #expect(try evaluate("today() + 1 month", context: context) == [.date(DateComponents(year: 2026, month: 9, day: 14))])
    ...
    @Test
    func ordinalValueScoresWithoutItemWeight() throws {
        let context = try Self.codingContext(extensions: [("ordinalValue", 3)])
        #expect(try evaluate("item.answer.weight()", context: context) == [.decimal(3)])
    }

    @Test
    func itemWeightWinsOverOrdinalValue() throws {
        let context = try Self.codingContext(extensions: [("ordinalValue", 3), ("itemWeight", 2)])
        #expect(try evaluate("item.answer.weight()", context: context) == [.decimal(2)])
    }


    /// Foundation boxes JSON booleans and numbers alike on Linux, where no CoreFoundation type check tells them apart.
    @Test
    func jsonBooleansAndNumbersKeepTheirTypes() throws {
        let node = try FHIRPathNode(jsonData: Data(#"{ "yes": true, "no": false, "one": 1, "zero": 0, "half": 0.5 }"#.utf8))
        #expect(node.children(named: "yes") == [.bool(true)])
        #expect(node.children(named: "no") == [.bool(false)])
        #expect(node.children(named: "one") == [.number(1)])
        #expect(node.children(named: "zero") == [.number(0)])
        #expect(node.children(named: "half") == [.number(Decimal(string: "0.5") ?? 0)])
    }
