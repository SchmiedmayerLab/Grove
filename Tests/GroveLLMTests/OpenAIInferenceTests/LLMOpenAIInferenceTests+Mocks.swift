...
extension LLMOpenAIInferenceTests {
    ...
    final class MockChatClient: @unchecked Sendable, LLMOpenAIChatClientProtocol {
        ...
        var createResponseHandler: ((Operations.createResponse.Input) async throws -> Operations.createResponse.Output)?
        ...
        func createResponse(_ input: Operations.createResponse.Input) async throws -> Operations.createResponse.Output {
            guard let handler = createResponseHandler else {
                fatalError("Mock handler not set!")
            }
            return try await handler(input)
        }
