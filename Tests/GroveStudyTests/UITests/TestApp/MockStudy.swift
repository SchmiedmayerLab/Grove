...
func mockStudy(revision: MockStudyRevision) throws -> StudyBundle { // swiftlint:disable:this function_body_length
    let definition = StudyDefinition(
        ...
        metadata: StudyDefinition.Metadata(
            ...
            explanationText: .init([.enUS: "This is a fake study, intended for testing the GroveStudy package."]),
            shortExplanationText: .init([.enUS: "GroveStudy fake test study"]),
    ...
    let url = URL.temporaryDirectory.appendingPathComponent(UUID().uuidString, conformingTo: .studyBundle)
