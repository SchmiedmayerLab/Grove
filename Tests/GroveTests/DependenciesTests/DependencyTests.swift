...
struct DependencyTests { // swiftlint:disable:this type_body_length
    ...
    func loadingAdditionalDependency() throws {
        let grove = Grove(standard: DefaultStandard(), modules: [OptionalModuleDependency()])
        
        var modules = grove.modules
        ...
        grove.loadModule(TestModule3())
        
        modules = grove.modules
    ...
    func impossibleUnloading() throws {
        ...
        let grove = Grove(standard: DefaultStandard(), modules: [TestModule1(), module3])
        
        // cannot unload module that other modules still depend on
        let moduleError = try #require(throws: GroveModuleError.self) {
            try grove._unloadModule(module3)
        }
    ...
    func multiLoading() throws {
        ...
        let grove = Grove(standard: DefaultStandard(), modules: [module])
        
        grove.unloadModule(module)
        grove.loadModule(module)
        grove.unloadModule(module)
    }
#endif
    ...
    func unloadingDependencies() async throws {
        func runUnloadingTests(deinitExpectation1: TestExpectation, deinitExpectation3: TestExpectation) throws -> Grove {
        ...
        // make sure we keep the reference to `Grove`, but loose all references to TestModule3 to test deinit getting called
        let grove = try runUnloadingTests(deinitExpectation1: deinitExpectation1, deinitExpectation3: deinitExpectation3)
        _ = grove // silence warning
    ...
    func selfManagedModules() async throws {
        ...
        func runModuleTests(deinitExpectation: TestExpectation) throws -> Grove {
            let module1 = TestModule1(deinitExpectation: deinitExpectation)
            
            let grove = Grove(standard: DefaultStandard(), modules: [optionalModule, moduleX, module8])
            
            grove.loadModule(module1, ownership: .external) // LOAD AS EXTERNAL
            #expect(Set(optionalModule.nums) == Set([5, 5, 4, 3, 2, 1, 9, 10, 11]))
            
            // leaving this scope causes the module1 to deallocate and should automatically unload it from Grove!
            #expect(grove.modules.count == 9)
            return grove
        }
        ...
        // make sure we keep the reference to `Grove`, but loose all references to TestModule3 to test deinit getting called
        let grove = try runModuleTests(deinitExpectation: deinitExpectation)
        _ = grove
        
        try await Task.sleep(for: .milliseconds(250)) // deinit need to get back to MainActor
        
        #expect(grove.modules.count == 5)
    ...
    func multipleDependenciesOfSameType() throws {
        ...
        let grove = Grove(standard: DefaultStandard(), modules: [first, second, TestModule4()])
    ...
    func unloadingWeakDependencyOfSameType() async throws {
        let grove = Grove(standard: DefaultStandard(), modules: [SimpleOptionalModuleDependency()])
        ...
        let scope = {
            ...
            grove.loadModule(weakModule6, ownership: .external)
            grove.loadModule(dynamicModule6)
            grove.loadModule(baseModule6)
        ...
        grove.unloadModule(dynamicModule6)
    ...
    func moduleLoadingOrderAndRequiredModulesBehavior() throws {
        let grove = Grove(standard: DefaultStandard(), modules: [ModuleWithRequiredDependency()])
    ...
    func injectionOfOptionalDependencyWithDefaultValue() throws {
        let grove = Grove(standard: DefaultStandard(), modules: [InjectionOfOptionalDefaultValue()])
    ...
    func deprecatedInits() throws {
        let grove = Grove(standard: DefaultStandard(), modules: [DeprecatedDeclarations()])
