...
class TestAppUITests: XCTestCase {
    ...
    func testMappingCompleteness() throws {
        ...
        //
        // `deferred` was the first group until the adapter stopped deferring anything, which left
        // the refusals first.
        XCTAssertTrue(app.staticTexts["intentionally-unsupported"].waitForExistence(timeout: 5))
        XCTAssertTrue(app.staticTexts["Activity Move Mode"].waitForExistence(timeout: 5))
    }
}
