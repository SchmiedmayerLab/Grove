...
struct HealthKitFHIRMobileCanonicalizationTests {
    ...
    func effectiveInstant(testCase: InstantCase) throws {
        let result = try HealthKitFHIRMobileCanonicalization.effectiveDateTime(
    ...
    func scalarDecimal() throws {
        let value = try HealthKitFHIRMobileCanonicalization.scalarDecimal(36.52)
    ...
    func nonFiniteValues() {
        #expect(throws: GroveHealthKitFHIRError.invalidValue) {
            try HealthKitFHIRMobileCanonicalization.scalarDecimal(.infinity)
        }
        #expect(throws: GroveHealthKitFHIRError.invalidValue) {
            try HealthKitFHIRMobileCanonicalization.scalarDecimal(.nan)
        }
        #expect(throws: GroveHealthKitFHIRError.invalidValue) {
            try HealthKitFHIRMobileCanonicalization.effectiveDateTime(
