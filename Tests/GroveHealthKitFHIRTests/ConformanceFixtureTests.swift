...
struct ConformanceFixtureTests {
    ...
    func writeConformanceFixtures() throws {
        ...
        let ecgContext = HealthKitFHIRConversionContext(
            subject: Self.subject,
        ...
        let ecgConversion = try HealthKitFHIRConverter.assembleGraph(
            for: ecgEnvelopeSource,
            context: ecgContext
        ) { recordingDeviceURL, converterURL in
            try HealthKitFHIRConverter.ecgObservation(
                input: ecgInput,
    ...
    func writeAuthoritativeSourceInventory() throws {
        let rows = try HealthKitFHIRCatalog.entries.map { entry in
            SourceInventory.Row(
