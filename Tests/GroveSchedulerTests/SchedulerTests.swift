...
// `SchedulerExitTests.sandboxDetection` used to assert that `.onDisk` trapped in a non-sandboxed macOS
// process. That guard existed only because the store lived in `~/Documents`; it now lives in the app's
// Application Support container, where non-sandboxed macOS is a supported configuration.
